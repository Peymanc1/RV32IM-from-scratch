// tb_rv32im_core.sv  -  single-cycle integration testbench
//
// Wires up the core with imem + dmem, runs the program in program.hex,
// and dumps every register at exit in a format compare_traces.py can parse.
//
// Exit conditions: a write to TOHOST_ADDR with value 1 (HTIF convention),
// or CYCLE_LIMIT cycles without progress.
...
module tb_rv32im_core;
    ...
    localparam logic [31:0] TOHOST_ADDR = 32'h8000_1000;     // Spike-compatible
    ...
    // ---- halt detect (HTIF tohost: write 1 = exit success) ----
    logic halted;
    ...
    // REGDUMP lines are picked up by compare_traces.py with a regex.
